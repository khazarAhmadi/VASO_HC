...
%% Script to run GLM on layers, normalizing the values
% This script is an adaptation of VPF_layer_pipeline_hippocampus_EPI.m from
% Viktor Pfaffenrot.For further details Check: 
% https://github.com/viktor-pfaffenrot/hippocampus_laminarfMRI_code/blob/main/layerfication/VPF_layer_pipeline_hippocampus_EPI.m 
...
for n = 1:N_sub
   ...
   % Do the z-transformation, by finding the mean and std of the volumes in math condition
   for run = 1:runs 
       idx = find(SPM.xX.X(SPM.Sess(run).row,SPM.Sess(run).col(2))>0);
       if run > 1
          idx = idx + (N_vol*(run-1));
       end  
       m = mean(layers_bold_per_sub{1,1}(:,idx,:),2);
       s = std(layers_bold_per_sub{1,1}(:,idx,:),[],2);
       layers_bold_per_sub{1,1}(:,run_indx_max(run,:),:) = (layers_bold_per_sub{1,1}(:,run_indx_max(run,:),:) - m)./s;
   end
   ...
   for f = 1: Subfield
      ...
      save(strcat('results_Ztransformed','_',string(f)),'-struct','results');
   end
...
for n = 1:N_sub
   ...
   for run = 1:runs 
       ...
       m = mean(layers_vaso_per_sub{1,1}(:,idx,:),2);
       s = std(layers_vaso_per_sub{1,1}(:,idx,:),[],2);
       layers_vaso_per_sub{1,1}(:,run_indx_max(run,:),:) = (layers_vaso_per_sub{1,1}(:,run_indx_max(run,:),:) - m)./s;
   end
